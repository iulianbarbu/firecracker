...
def run_fio(env_id, basevm, ssh_conn, mode, bs):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        rc, stdout, stderr = ssh_conn.execute_command(cmd)
        assert rc == 0, stderr.read()
        assert stderr.read() == ""
