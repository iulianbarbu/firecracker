...
impl ConnectionTuple {
    fn new(remote_addr: Ipv4Addr, remote_port: u16, local_addr: Ipv4Addr, local_port: u16) -> Self {
        ConnectionTuple {
            remote_addr,
            remote_port,
            local_addr,
...
pub struct TcpIPv4Handler {
    // Used for every connection tuple as port for the MMDS communication end.
    local_port: u16,
    ...
    pub fn write_next_packet(
        ...
        if let Some((tuple, rst_cfg)) = self.rst_queue.pop() {
            ...
            .finalize(
                tuple.local_port,
                tuple.remote_port,
                Some((tuple.local_addr, tuple.remote_addr)),
            )
            .len();

            packet
                .inner_mut()
                .set_source_address(tuple.local_addr)
...
mod tests {
    ...
    fn test_handler() {
        ...
        let remote_tuple = ConnectionTuple::new(remote_addr, remote_port, local_addr, local_port);
        let remote_tuple2 =
            ConnectionTuple::new(remote_addr, remote_port + 1, local_addr, local_port);
        ...
        if let Some((_, tuple)) = h.next_timeout {
            assert_ne!(
                tuple,
                ConnectionTuple::new(remote_addr, remote_port, local_addr, local_port)
            );
        } else {
