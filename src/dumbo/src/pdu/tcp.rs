...
impl<'a, T: NetworkBytesMut> TcpSegment<'a, T> {
    ...
    pub fn write_segment<R: ByteBuffer + ?Sized>(
        ...
        let (written_count, segment) = Self::write_incomplete_segment(
            buf,
            seq_number,
            ack_number,
            flags_after_ns,
        ...
        Ok((written_count, segment.finalize(src_port, dst_port, compute_checksum)))
    }
    ...
    pub fn write_incomplete_segment<R: ByteBuffer + ?Sized>(
        ...
        mss_remaining: u16,
        payload: Option<(&R, usize)>,
    ) -> Result<(usize, Incomplete<Self>), Error> {
        ...
        let written_count =  if let Some((payload_buf, max_payload_bytes)) = payload {
        ...
        Ok((written_count, Incomplete::new(segment)))
    }
}
...
mod tests {
    ...
    fn test_constructors() {
        ...
        let segment_len = {
            let (_, mut segment) = TcpSegment::write_segment(
                a.as_mut(),
                src_port,
                dst_port,
                seq_number,
        ...
        {
            let segment = TcpSegment::from_bytes(&a[..segment_len], Some((src_addr, dst_addr))).unwrap();
            assert_eq!(
        ...
        {
            let (_, segment) = TcpSegment::write_segment(
                a.as_mut(),
                src_port,
                dst_port,
                seq_number,
